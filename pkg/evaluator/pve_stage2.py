...
class PVEStage2Evaluator(PVEEvaluator):
    keeper_path: str = "./keepers/stage2"
    ...
    def _start_baseline_teams(self) -> List[multiprocessing.Process]:
        ...
        ret = os.system(
            f"tar -xf {self.keeper_path}-1.tar -C {os.path.dirname(self.keeper_path)}"
        )
        if ret:
            raise f"untar keeper policy failed"

        ret = os.system(
            f"tar -xf {self.keeper_path}-2.tar -C {os.path.dirname(self.keeper_path)}"
        )
        if ret:
            raise f"untar keeper policy failed"

        ret = os.system(
            f"tar -xf {self.keeper_path}-3.tar -C {os.path.dirname(self.keeper_path)}"
        )
        if ret:
            raise f"untar keeper policy failed"

        team1 = submission.get_team_from_submission(
            f"{self.keeper_path}-1",
            "OldHand(小巧)",
            self.env_config,
        )
        team2 = submission.get_team_from_submission(
            f"{self.keeper_path}-2",
            "Competent(用智)",
            self.env_config,
        )
        team3 = submission.get_team_from_submission(
            f"{self.keeper_path}-3",
            "General(通幽)",
            self.env_config,
        )

        ps = []
        ps.extend(self._start_teams([team1] * 7))
        ps.extend(self._start_teams([team2] * 5))
        ps.extend(self._start_teams([team3] * 3))
        return ps

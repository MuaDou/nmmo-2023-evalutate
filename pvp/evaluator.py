...
class Constants:
    ...
    MODE: Mode = Mode(os.getenv("NMMO_MODE", 'PVP'))
...
class AIcrowdEvaluator:
    ...
    @staticmethod         
    def folder_check():
        if os.path.exists( Constants.NMMO_BASE_POLICY_STORE_DIR ):
            print( 'Remove Previous Policy Store Dir' )
            # os.rmdir( policy_store_dir )
            os.system( f'rm -rf {Constants.NMMO_BASE_POLICY_STORE_DIR}' )
        print( 'Creat Policy Store Dir' )
        os.makedirs( Constants.NMMO_BASE_POLICY_STORE_DIR )
        
        if os.path.exists( Constants.NMMO_SUBMISSION_DIR ):
            print( 'Remove Previous Submissions Dir' )
            # os.rmdir( submissions_dir )
            os.system( f'rm -rf {Constants.NMMO_SUBMISSION_DIR}' )
        print( 'Creat Submissions Dir' )
        os.makedirs( Constants.NMMO_SUBMISSION_DIR )
        
        if os.path.exists( Constants.SHARED_DIR ):
            print( 'Remove Previous Share Dir' )
            # os.rmdir( shared_dir )
            os.system( f'rm -rf {Constants.SHARED_DIR}' )
        print( 'Creat Share Dir' )
        os.makedirs( Constants.SHARED_DIR )
